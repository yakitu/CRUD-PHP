...
<body>
  ...
  if ($_SESSION['admin'] == 1) { //Si l'usuari vigent és administrador
    ...
    $sql_total = "select * from Procuradors";
    $resultat = $baseDades->prepare($sql_total);
    $resultat->execute(array());
    $num_files = $resultat->rowCount();
    $resultat->closeCursor();
    $total_pagines = ceil($num_files / $tamany_pagines);
    $connexio = $baseDades->query("select * from Procuradors limit $iniciar_des_de, $tamany_pagines");
    ...
    if (isset($_POST["cr"])) {
      ...
      $adr = $_POST["adr"];
      $ccpp = $_POST["ccpp"];
      $sql = "insert into Procuradors (dni, nom, cognom1, cognom2, telèfon, email, adreça, CCPP)
  ...
  ?>
    <h1>CRUD 1.0<span class="subtitol"> Procuradors</span></h1>
    <h3 class="centrat">Introduïu les dades per crear un nou procurador</h3>
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
      <table width="50%" border="0" align="center">
        ...
        <?php
        foreach ($registres as $procurador) :
        ?>
          <tr>
            <td><?php echo $procurador->id ?></td>
            <td><?php echo $procurador->dni ?></td>
            <td><?php echo $procurador->nom ?></td>
            <td><?php echo $procurador->cognom1 ?></td>
            <td><?php echo $procurador->cognom2 ?></td>
            <td><?php echo $procurador->telèfon ?></td>
            <td><?php echo $procurador->email ?></td>
            <td><?php echo $procurador->adreça ?></td>
            <td><?php echo $procurador->CCPP ?></td>
            <td class="bot"><a href="esborraProcuradors.php?id=<?php echo $procurador->id ?>">
                <input type='button' name='del' id='del' value='Esborrar'></a></td>
            <td class='bot'><a href="editaProcuradors.php?id=<?php echo $procurador->id ?>
          &dni=<?php echo $procurador->dni ?>&nom=<?php echo $procurador->nom ?>
          &cog1=<?php echo $procurador->cognom1 ?>&cog2=<?php echo $procurador->cognom2 ?>
          &tel=<?php echo $procurador->telèfon ?>&ema=<?php echo $procurador->email ?>
          &adr=<?php echo $procurador->adreça ?>&ccpp=<?php echo $procurador->CCPP ?>">
  ...
  <?php
    ...
    echo "<h4>Aquest usuari no està autoritzat per administrar procuradors</h4>";
  }
  ?>
</body>
